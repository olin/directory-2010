...
class HTTPManager {
	...
	public static function requireSSL($redirectToSSL=true){
		//site-config SSL override?
		if(Config::get('site.ssl')=="disabled"){ return; }
		//Only act if SSL is already on
	...
	/* requires page to be accessed via HTTP (SSL *not* allowed)
	 * IMPORTANT: Redirecting POST requests may not preserve all POST data
	 */
	public static function forbidSSL($redirectToPlain=true){
		if(@$_SERVER['HTTPS']!="on"){ return Status::ok(); }
		//build an error message
		if($redirectToPlain){
		   header("HTTP/1.1 307 Temporary Redirect"); //necessary to make browsers re-POST form data
		   ...
		   die();
			...
			return Status::error("The specified action *cannot* be performed over an SSL-encrypted session (HTTPS)");
			}
		return Status::ok();
		}
